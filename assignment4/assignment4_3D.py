...
train_with_label = pd.DataFrame(data = train_with_label , columns = ['x', 'y', 'z', 'labels'])
test_with_label = pd.DataFrame(data = test_with_label , columns = ['x', 'y','z', 'labels'])
...
ax.set_title('3 Component T-SNE for Train data', fontsize = 20)
ax.set(xlim=[-60., 60.], ylim=[ -60., 60.], zlim=[-60., 60.])

for label in range(10):
    indice = train_with_label['labels'] == label_decodes[label]
    ax.scatter(train_with_label.loc[indice, 'x'], train_with_label.loc[indice, 'y'], train_with_label.loc[indice, 'z'], s = 30)
ax.legend(labels)
ax.grid()
...
train_2D = pd.DataFrame(train_2D, columns = ['x', 'y', 'z'])
...
ax2.set(xlim=[-60., 60.], ylim=[-60., 60.], zlim=[-60., 60.])

for label in range(10):
    indice = result['cluster'] == label
    ax2.scatter(result.loc[indice, 'x'], result.loc[indice, 'y'],result.loc[indice, 'z'], s = 30)
ax2.scatter(centroids['x'], centroids['y'],centroids['z'], c = 'black')
ax2.grid()
plt.show()
